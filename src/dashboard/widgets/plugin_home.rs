...
impl PluginHomeWidget {
    ...
    fn update_actions(
        ...
        if let Some(loader) = self.retiring_loader.as_mut() {
            retire_finished = loader.poll().is_some();
            if let Some(failure) = loader.take_failure() {
                self.loader_error = Some(format!("Retired plugin executor failed: {failure}"));
...
mod tests {
    ...
    fn panicking_plugin_latches_same_source_and_identity_change_recovers() {
        ...
        while widget.cached_source.as_ref() != Some(&recovered_source) {
            widget.update_actions(
