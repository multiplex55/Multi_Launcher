            ...
            .show(ctx, |ui| {
                ...
                if response.drag_started() {
                    if ctx.input(|i| i.modifiers.shift) {
                        self.ann_start = response.interact_pointer_pos();
                    } else {
                        self.crop_start = response.interact_pointer_pos();
                        self.crop_rect = None;
                    }
                }
                if response.dragged() {
                    if let Some(start) = self.crop_start {
                        if let Some(pos) = response.interact_pointer_pos() {
                            self.crop_rect = Some(Rect::from_two_pos(start, pos));
                        }
                    }
                    if let Some(start) = self.ann_start {
                        if let Some(pos) = response.interact_pointer_pos() {
                            self.annotations.push(Rect::from_two_pos(start, pos));
                            self.ann_start = None;
                        }
                    }
                }
                ...
                if let Some(rect) = self.crop_rect {
                    painter.rect_stroke(rect, 0.0, Stroke::new(1.0, Color32::GREEN));
                }
                for rect in &self.annotations {
