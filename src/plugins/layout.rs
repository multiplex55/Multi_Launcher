        ...
        if rest.is_empty() {
            return vec![
                Action {
                    ...
                    args: None,
                    preview_text: Some("Query shortcut for layout operations.".into()),
                ...
                Action {
                    ...
                    args: None,
                    preview_text: Some("Layout command shortcut.".into()),
                    risk_level: Some(ActionRiskLevel::Low),
                    icon: Some("layout".into()),
                },
                Action {
            ...
            if let Ok(store) = load_layouts(layouts_config_path()) {
                return store
                    ...
                    .map(|layout| Action {
                        ...
                        args: None,
                        preview_text: Some("Repositions windows based on the saved layout.".into()),
            ...
            if let Ok(store) = load_layouts(layouts_config_path()) {
                        ...
                        return layout
                            ...
                            .map(|(idx, window)| {
                                ...
                                Action {
                                    ...
                                    args: None,
                                    preview_text: Some(
                                        "Preview row for a window match inside this layout.".into(),
                                    ),
                                    risk_level: Some(ActionRiskLevel::Low),
                                    icon: Some("layout".into()),
                                }
                ...
                return store
                    ...
                    .map(|layout| Action {
                        ...
                        args: None,
                        preview_text: Some("Shows the windows included in this layout.".into()),
                        risk_level: Some(ActionRiskLevel::Low),
                        icon: Some("layout".into()),
                    })
                    .collect();
