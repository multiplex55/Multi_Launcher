...
fn mouse_gestures_commands_match_expected_labels() {
    ...
    assert_eq!(labels, vec!["mg", "mg settings", "mg edit", "mg add", "mg list"]);
...
fn mouse_gestures_default_settings_round_trip() {
    ...
    let parsed: MouseGestureSettings = serde_json::from_value(value.clone())
        .expect("deserialize mouse gesture settings");
